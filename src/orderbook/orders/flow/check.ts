...
export const offChainCheck = async (
  ...
  const minNonce = await commonHelpers.getMinNonce("flow", order.signer);
  const isNonceCancelled = await commonHelpers.isNonceCancelled("flow", order.signer, order.nonce);
  ...
  if (order.isSellOrder) {
    const operator = Sdk.Flow.Addresses.Exchange[config.chainId];
