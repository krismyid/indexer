...
describe("Flow", () => {
  ...
  const exchangeAddress = Flow.Addresses.Exchange[config.chainId];
  ...
  test("takeSellERC721", async () => {
    ...
    const startTime = now();
    const sellOrder = new Flow.Builders.SingleToken(chainId).build({
      isSellOrder: true,
    ...
    logger.info("Flow", `Save ${orderId} to database`);
    const result = await orders.flow.save([orderInfo]);
    expect(result[0]?.status).toEqual("success");

    logger.info("FlowTestnet", `Waiting... ${indexInterval}`);
    ...
    logger.info("FlowTestnet", `Order status ${JSON.stringify(orderStatus, null, 2)}`);
    expect(orderStatus?.fillability_status).toEqual("fillable");

    const fillTx = await exchange.takeMultipleOneOrders(buyer, sellOrder);
    logger.info("FlowTestnet", `Fill order=${orderId}, tx=${fillTx.hash}`);
    await fillTx.wait();
    logger.info("FlowTestnet", `Waiting... ${indexInterval}`);
    await wait(indexInterval);
  ...
  test("takeOfferERC721", async () => {
    ...
    const startTime = now();
    const price = parseEther("0.001").toString();
    const offerOrder = new Flow.Builders.SingleToken(chainId).build({
      isSellOrder: false,
  ...
  test("cancelOrderViaCancelMultiple", async () => {
    ...
    logger.info("FlowTestnet", `Cancel order=${orderId}, tx=${cancelTx.hash}`);
    await cancelTx.wait();
    logger.info("FlowTestnet", `Waiting... ${indexInterval}`);
    await wait(indexInterval);
...
async function getMinOrderNonce(user: Wallet, exchange: Flow.Exchange) {
  ...
  if (minNonce.gt(minOrderNonce)) {
    try {
      ...
      logger.warn("FlowTestnet", `Error cancelling order ${err}`);
    }
  }
